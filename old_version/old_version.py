...
def save_data(data):
    ...
    print("📤 Saving tracking data:", data)
            ...
            if str(data.get("owner")) == user_id:
                ...
                if admin_channel:
                    await admin_channel.send(
                        f"✏️ **EDITED DM from <@{user_id}>** regarding character **{char_name}**:\n"
                        f"> {after.content}"
                    )
                break
    ...
    if isinstance(message.channel, discord.DMChannel) and not message.author.bot:
        ...
        for char_name, data in inactivity_tracker.items():
            if str(data.get("owner")) == user_id:
                ...
                if admin_channel:
                    await admin_channel.send(
                        f"🗑️ **DELETED DM from <@{user_id}>** regarding character **{char_name}**:\n"
                        f"> {deleted_text}"
                    )
                break
...
async def on_message(message):
    ...
    # ✅ Auto-track new threads if created in a tracked category
    if message.type == discord.MessageType.thread_created and message.channel.parent_id in admin_tracked_categories:
        if hasattr(message.channel, "name"):
            ...
            print("🧵 New thread created in tracked category (channel has no name)")
    ...
    # ✅ Handle Tupperbox/webhook activity
    if message.webhook_id:
        ...
        for user_id in tracking_data:
            ...
            # ✅ Auto-track thread like /trackthread if it's missing
            if thread_id not in user_threads:
                channel_name = getattr(message.channel, "name", "Unnamed Channel")
                print(f"📌 Auto-tracking new thread for {user_id}: {channel_name}")
        ...
        # ✅ Archive the forum if the character is inactive
        await archive_forum(key)

        # ✅ Save after any Tupper message
        save_data(tracking_data)
        save_characters()
    ...
    if forum_channel_id:
        try:
            ...
            if archive_category:
                ...
                await forum_channel.edit(category=archive_category, reason="Moved to archive — inactive character.")
                await forum_channel.edit(name=f"archived-{forum_channel.name}")  # Optional renaming
                print(f"✅ Forum {forum_channel.name} moved to archive.")
            else:
...
async def deletecharacter(interaction: discord.Interaction, name_or_owner: str, confirm: bool = False):
    ...
    await interaction.response.send_message(
        f"⚠️ Character **{base_name}** not found. Make sure the name is correct or check the list of registered characters.", ephemeral=True)
    ...
    while not client.is_closed():
        try:
            ...
            for char_name, info in characters_data.get("activity", {}).items():
                ...
                await user.send(
                    f"👋 Hi! Your character **{base_name}** has been inactive for "
...
async def accept_user(interaction: discord.Interaction, user: discord.Member, forum_post: str):
    ...
    # ✅ If no warnings exist, initialize with 0
...
class CheckSetupButtons(discord.ui.View):
    ...
    async def no_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if forum_channel_id:
            try:
                await archive_forum_channel(forum_channel_id, "Moved to archive — user didn't finish setup.")
            except Exception as e:
                print(f"❌ Failed to archive forum: {e}")
...
async def archive_forum_channel(channel_id: int, reason: str = "Archived"):
    try:
        ...
        if archive_category:
            ...
            print(f"✅ Forum {forum_channel.name} archived.")
        else:
            print(f"⚠️ Archive category not found for channel ID {channel_id}.")
    except Exception as e:
        print(f"❌ Failed to archive forum (Channel ID: {channel_id}): {e}")
